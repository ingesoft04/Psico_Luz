...
const auditClinical = require('../middleware/auditClinical');
const { assertUuid, assertClinicalAuthor, normalizeClinicalNote, UUID_PATTERN } = require('../utils/clinicalNote');

router.use(auth, rol('psicologa'));

const patientByAppointment = async req => {
  if (!UUID_PATTERN.test(req.params.citaId || '')) return null;
  const { rows } = await db.query('SELECT paciente_id FROM app.citas WHERE id=$1', [req.params.citaId]);
  return rows[0]?.paciente_id || null;
};
const patientByNote = async req => {
  if (!UUID_PATTERN.test(req.params.notaId || '')) return null;
  const { rows } = await db.query('SELECT paciente_id FROM app.notas_sesion WHERE id=$1', [req.params.notaId]);
  return rows[0]?.paciente_id || null;
};
const auditAppointment = auditClinical('registro_sesion_por_cita', patientByAppointment);
const auditDocument = auditClinical('documento_clinico', patientByNote);
...
router.get('/cita/:citaId/contexto', auditAppointment, async (req, res) => {
  assertUuid(req.params.citaId, 'Cita');
  const { rows } = await db.query(`SELECT c.id,c.fecha,c.hora_inicio,c.hora_fin,c.modalidad,
    ...
    n.id nota_id,n.profesional_id,n.contenido,n.estado_animo,n.progreso,n.tareas_asignadas,n.next_steps,
  ...
  assertClinicalAuthor(rows[0], req.user.id);
  res.json({ ok: true, data: rows[0] });
});

router.get('/:citaId', auditAppointment, async (req, res) => {
  assertUuid(req.params.citaId, 'Cita');
  const { rows } = await db.query('SELECT * FROM app.notas_sesion WHERE cita_id=$1 AND profesional_id=$2 ORDER BY created_at DESC', [req.params.citaId, req.user.id]);
  res.json({ ok: true, data: rows });
});

router.post('/:citaId', auditAppointment, async (req, res) => {
  assertUuid(req.params.citaId, 'Cita');
  const { contenido, estado_animo, progreso, tareas_asignadas, next_steps } = normalizeClinicalNote(req.body);
...
router.put('/documento/:notaId', auditDocument, async (req, res) => {
  assertUuid(req.params.notaId, 'Nota');
  ...
  assertClinicalAuthor(current.rows[0], req.user.id);
  const { contenido, estado_animo, progreso, tareas_asignadas, next_steps } = normalizeClinicalNote(req.body);
...
router.post('/documento/:notaId/firmar', auditDocument, async (req, res) => {
  assertUuid(req.params.notaId, 'Nota');
  const note = await fullDocument(req.params.notaId);
  assertClinicalAuthor(note, req.user.id);
...
router.post('/documento/:notaId/proteger', auditDocument, async (req, res) => {
  assertUuid(req.params.notaId, 'Nota');
  const note = await fullDocument(req.params.notaId);
  assertClinicalAuthor(note, req.user.id);
...
router.get('/documento/:notaId/verificar', auditDocument, async (req, res) => {
  assertUuid(req.params.notaId, 'Nota');
  const note = await fullDocument(req.params.notaId);
  assertClinicalAuthor(note, req.user.id);
...
router.get('/documento/:notaId/pdf', auditDocument, async (req, res) => {
  assertUuid(req.params.notaId, 'Nota');
  const note = await fullDocument(req.params.notaId);
  assertClinicalAuthor(note, req.user.id);
...
router.post('/documento/:notaId/enviar', auditDocument, async (req, res) => {
  assertUuid(req.params.notaId, 'Nota');
  const note = await fullDocument(req.params.notaId);
  assertClinicalAuthor(note, req.user.id);
